...
9. **Outputs** — rotated full-resolution PNG for slides and rotated UINT16/19-channel OME-TIFF for archive.
...
The rotated full-resolution PNG is the visual output for contact sheets, figures, and slide presentations. The rotated multichannel OME-TIFF preserves UINT16 bit depth and every original channel while baking the accepted transform into each plane. Rotation requires interpolation, so the original whole slide remains the immutable scientific source of truth.
