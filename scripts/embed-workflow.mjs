import { readFile, writeFile } from "node:fs/promises";
import { gzipSync } from "node:zlib";

// The seven numbered steps are the real source of the workflow. They live in
// workflow/embedded/ and are gzip/base64-embedded into workflow/CoreAlign.groovy
// so a run needs one file and nothing else. Edit the .src file, run this script,
// commit both.
const root = new URL("../", import.meta.url);
const runnerUrl = new URL("workflow/CoreAlign.groovy", root);
const sources = [
  ["1", "01_build_tma_grid.groovy"],
  ["2", "02_auto_orient_epidermis.groovy"],
  ["3", "03_review_correct_and_approve_grid.groovy"],
  ["4", "04_restore_approved_grid.groovy"],
  ["5", "05_finalize_orientation_review.groovy"],
  ["6", "06_export_presentation_package.groovy"],
  ["7", "07_build_qupath_analysis_project.groovy"],
];

const wrapBase64 = (value) => value.match(/.{1,76}/g).join("\n");
let runner = await readFile(runnerUrl, "utf8");

for (const [step, name] of sources) {
  const source = await readFile(new URL(`workflow/embedded/${name}.src`, root), "utf8");
  const payload = wrapBase64(gzipSync(Buffer.from(source, "utf8"), { level: 9 }).toString("base64"));
  const pattern = new RegExp(
    `(def step${step} = new EmbeddedWorkflowScript\\(name: '${name.replaceAll(".", "\\.")}', payload: '''\\n)[\\s\\S]*?(\\n'''\\))`,
  );
  if (!pattern.test(runner)) throw new Error(`Could not find embedded step ${step}: ${name}`);
  runner = runner.replace(pattern, `$1${payload}$2`);
}

const placeholderJpeg = await readFile(new URL("workflow/assets/no-core-placeholder.jpg", root));
const placeholderPayload = wrapBase64(placeholderJpeg.toString("base64"));
const placeholderPattern = /(String NO_CORE_PLACEHOLDER_JPEG_BASE64 = '''\n)[\s\S]*?(\n''')/;
if (!placeholderPattern.test(runner)) throw new Error("Could not find the no-core placeholder payload");
runner = runner.replace(placeholderPattern, `$1${placeholderPayload}$2`);

await writeFile(runnerUrl, runner, "utf8");
console.log(`Embedded ${sources.length} workflow steps into workflow/CoreAlign.groovy`);
