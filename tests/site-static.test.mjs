...
test("includes persistent navigation and a theme control on both pages", async () => {
  ...
  assert.match(docs, /QuPath can stay closed until you are ready to run CoreAlign again/);
  ...
  assert.match(docs, /browser updates the config directly/);
...
test("ships one guarded production workflow", async () => {
  ...
  assert.match(groovy, /Open QuPath and run CoreAlign when you are ready to create multichannel OME-TIFF files/);
  assert.match(groovy, /Rotation saved/);
  assert.match(groovy, /Choose the folder that contains REPORT\.html/);
  ...
  assert.match(groovy, /Saved in the project folder\. Run CoreAlign when ready/);
  assert.doesNotMatch(groovy, /Keep QuPath open/);
