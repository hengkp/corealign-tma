...
// Render nuclear signal as white on black with one slide-level calibration.
// Colored grid annotations are drawn afterwards and remain easy to distinguish.
def renderSlideQcRgb = { BufferedImage raw, List names ->
    ...
    def nuclearTokens = ['dapi', 'hoechst', 'nuclear', 'draq', 'topro']
    def chosen = stats.findAll { stat ->
        String name = stat.name.toString().toLowerCase()
        nuclearTokens.any { token -> name.contains(token) }
    }
    if (chosen.isEmpty())
        chosen << stats.max { a, b -> (a.span as double) <=> (b.span as double) }
    chosen = chosen.take(8)
    ...
    chosen.each { stat ->
        ...
        for (int i = 0; i < n; i++) {
            ...
            signal = Math.pow((signal - 0.015d) / 0.985d, 0.62d)
            if (signal > strongest[i]) strongest[i] = (float) signal
        }
    }
    int[] pixels = new int[n]
    for (int i = 0; i < n; i++) {
        int white = Math.min(255, Math.max(0,
            (int) Math.round(255.0d * strongest[i])))
        pixels[i] = (white << 16) | (white << 8) | white
    }
    def out = new BufferedImage(w, h, BufferedImage.TYPE_INT_RGB)
    out.setRGB(0, 0, w, h, pixels, 0, w)
    println "Grid QC white-on-black nuclear channels: ${chosen.collect { it.name }.join(', ')}"
    return out
}
