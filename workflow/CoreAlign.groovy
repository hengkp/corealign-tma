...
function updateChangeBar(){var count=document.querySelectorAll('.core-card[data-has-change="true"]').length,confirmedCount=document.querySelectorAll('.core-card[data-confirmed="true"]').length,hasReview=count>0||confirmedCount>0,bar=document.getElementById("changeBar"),label=document.getElementById("changeCount"),errorVisible=autoSaveStatus&&autoSaveStatus.dataset.state==="error";if(bar){bar.hidden=!hasReview||(errorVisible&&autoSaveErrorHideTimer===0);}if(label){label.textContent=count>0?count+" angle change"+(count===1?"":"s"):"Review saved";}if(openQuPath){openQuPath.hidden=!correctionOpenQuPathUrl;}if(hasReview&&autoSaveStatus&&autoSaveStatus.dataset.state==="ready"){setAutoSaveStatus(count>0?(correctionAutoSaveUrl?"Saved. Go to QuPath and run CoreAlign again.":"Open QuPath and run CoreAlign once to enable auto-save."):"Go to QuPath and run CoreAlign again.",count>0&&!correctionAutoSaveUrl?"error":"ready",count>0&&!correctionAutoSaveUrl);}}
