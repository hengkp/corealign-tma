...
qc/02-orientation   Check rotation, crop, confirmed changes, and machine-readable run data.
...
def writeProjectIndex = { File runDir = null ->
    ...
    String nextAction = hasCompletion ?
        ...
        hasOrientation ?
            'Check the flagged cores below. Use Edit only where a rotation needs to change.' :
        hasGridQc ?
            'Check every detected circle and missing position in QuPath, then run the same script again.' :
...
@media(prefers-reduced-motion:reduce){*,*::before,*::after{animation-duration:.01ms!important;animation-iteration-count:1!important;transition-duration:.01ms!important;scroll-behavior:auto!important}}
    ...
    html.append('</div>')
    html.append('<div class="notice">').append(projectIcon('warning')).append('<div><strong>Wrong rotation?</strong><p>Click Edit, adjust the slider, and Confirm. Download the changes, then run CoreAlign again. Only edited cores are rebuilt.</p></div></div>')
    if (hasOrientation && !coreRecords.isEmpty()) {
        html.append('<div class="filter-tools section-block"><div class="filterbar" role="group" aria-label="Filter core review cards"><button type="button" data-filter="review" aria-pressed="')
            ...
            .append(reviewCountForPage > 0 ? 'false' : 'true').append('">QC pass</button><button type="button" data-filter="all" aria-pressed="false">All cores</button><button type="button" data-filter="changes" aria-pressed="false">Changes</button></div><label class="search-control"><span class="sr-only">Find a core by row and column</span><input class="core-search" id="coreSearch" type="search" placeholder="Find core, for example 4-C"><button class="search-clear" type="button" id="coreSearchClear" hidden>Clear</button></label></div><div class="core-grid" id="coreGrid">')
        coreRecords.each { record ->
            ...
            html.append('<article class="core-card" data-status="').append(projectHtmlEscape(status))
                ...
                .append(projectHtmlEscape(coreNameForSearch.toLowerCase())).append('" data-has-change="false" data-applied-adjustment="')
            ...
            html.append('</p>')
            if (!preview.isEmpty()) {
                html.append('<div class="card-actions"><span class="change-badge">Changed</span><button class="control-button edit-button" type="button" data-edit>Edit</button></div><div class="edit-panel"><div class="rotation-editor"><label>Rotation <output data-rotation-value>0 deg</output></label><input type="range" min="-180" max="180" step="1" value="0" data-rotation-adjust aria-label="Rotation adjustment for core ')
        ...
        html.append('</div><div class="change-bar" id="changeBar" hidden><strong id="changeCount">0 changes ready</strong><button class="primary" type="button" id="downloadChanges">Download changes</button></div>')
    ...
    html.append('<section class="panel" id="help" data-panel><div class="section-head"><div><div class="eyebrow">Quick guide</div><h2>Run, review, continue</h2><p>The workflow always uses the same script and the same START-HERE.html dashboard.</p></div></div><div class="help-list"><article class="help-item"><h3>Open the slide</h3><p>Keep the slide, CoreAlign.groovy, and optional corealign.config.json in this project folder. Open the slide in QuPath.</p></article><article class="help-item"><h3>Run CoreAlign.groovy</h3><p>Detection, rotation, cropping, exports, and safe resume are handled automatically. Stop points are intentional human review gates.</p></article><article class="help-item"><h3>Use this dashboard</h3><p>Open START-HERE.html after every run. Use the top menu, Back and Forward controls, or the Previous and Next buttons. Images open in a new tab, so this page stays available.</p></article><article class="help-item"><h3>Correct only what changed</h3><p>Use Edit for rotation. Use QuPath annotations for detection or crop corrections. Then run the same script again. Accepted cores are reused.</p></article></div><div class="section-block notice">').append(projectIcon('check')).append('<div><strong>No report filenames to remember</strong><p>START-HERE.html is the only workflow HTML. Machine-readable audit files stay in JSON and CSV format.</p></div></div><div class="pager"><button class="secondary" type="button" data-next="-1">').append(projectIcon('back')).append(' Results</button><button class="secondary" type="button" data-nav="overview">').append(projectIcon('home')).append(' Overview</button></div></section>')
...
function applyCoreFilters(){var query=coreSearch?coreSearch.value.trim().toLowerCase():"";document.querySelectorAll(".core-card").forEach(function(card){var statusOk=activeCoreFilter==="all"||(activeCoreFilter==="changes"?card.dataset.hasChange==="true":card.dataset.filterStatus===activeCoreFilter),nameOk=!query||(card.dataset.coreName||"").indexOf(query)>=0;card.hidden=!(statusOk&&nameOk);});}
...
var orientationSection=document.getElementById("orientation"),reviewStorageKey="corealign-rotation-edits:"+location.pathname+":"+(orientationSection?orientationSection.dataset.reviewKey:"pending"),reviewState={angles:{}};try{var savedReview=JSON.parse(localStorage.getItem(reviewStorageKey)||"{}");reviewState.angles=savedReview.angles||{};}catch(e){reviewState={angles:{}};}
...
function updateChangeBar(){var count=document.querySelectorAll('.core-card[data-has-change="true"]').length,bar=document.getElementById("changeBar"),label=document.getElementById("changeCount");if(bar){bar.hidden=count===0;}if(label){label.textContent=count+" change"+(count===1?"":"s")+" ready";}}
