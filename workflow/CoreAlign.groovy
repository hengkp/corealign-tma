...
/**
 * Loopback fallback for browsers that cannot grant folder access directly.
 * Modern browsers and AppHub save from REPORT.html without keeping QuPath open.
 */
...
:root{color-scheme:light;--bg:#ffffff;--surface:#f8f9fa;--surface-strong:#eef2f7;--fg:#202124;--muted:#5f6368;--border:#dadce0;--accent:#4285f4;--accent-deep:#1967d2;--accent-ink:#ffffff;--warning:#b06000;--warning-bg:#fef7e0;--danger:#d93025;--danger-bg:#fce8e6;--success:#188038;--success-bg:#e6f4ea;--purple:#4285f4;--purple-bg:#e8f0fe;--cyan:#4285f4;--cyan-bg:#e8f0fe;--coral:#ea4335;--coral-bg:#fce8e6;--yellow:#fbbc04;--yellow-bg:#fef7e0;--shadow:0 1px 2px rgb(60 64 67/.12);--radius-sm:6px;--radius-md:8px;--radius-lg:10px;--header:72px}
html[data-theme="dark"]{color-scheme:dark;--bg:#202124;--surface:#292a2d;--surface-strong:#35363a;--fg:#f1f3f4;--muted:#bdc1c6;--border:#5f6368;--accent:#8ab4f8;--accent-deep:#aecbfa;--accent-ink:#202124;--warning:#fdd663;--warning-bg:#4a3c16;--danger:#f28b82;--danger-bg:#4c2725;--success:#81c995;--success-bg:#20382a;--purple:#8ab4f8;--purple-bg:#25324a;--cyan:#8ab4f8;--cyan-bg:#25324a;--coral:#f28b82;--coral-bg:#4c2725;--yellow:#fdd663;--yellow-bg:#4a3c16;--shadow:0 1px 2px rgb(0 0 0/.4)}
...
@media(prefers-reduced-motion:reduce){*,*::before,*::after{animation-duration:.01ms!important;animation-iteration-count:1!important;transition-duration:.01ms!important;scroll-behavior:auto!important}}
    ...
    html.append('</div>')
    html.append('<div class="notice">').append(projectIcon('warning')).append('<div><strong>Confirm or edit</strong><p>Correct? Click Confirm. Wrong? Click Edit, set the angle, then Update. Chrome, Edge, and AppHub save directly to the project folder. Run CoreAlign again when you are ready.</p></div></div>')
    if (hasOrientation && !coreRecords.isEmpty()) {
        ...
        html.append('</div><div class="change-bar" id="changeBar" hidden><div class="change-copy"><strong id="changeCount">0 angle changes</strong><span class="autosave-status" id="autosaveStatus" data-state="ready">Saved in the project folder. Run CoreAlign when ready.</span></div><div class="change-actions"><button class="secondary" type="button" id="downloadChanges" hidden>Choose project folder</button><button class="primary" type="button" id="openQuPath">Open QuPath</button></div></div>')
...
var autoSaveErrorHideTimer=0;function showAutoSaveErrorBriefly(){var bar=document.getElementById("changeBar");if(!bar){return;}bar.hidden=false;if(autoSaveErrorHideTimer){clearTimeout(autoSaveErrorHideTimer);}autoSaveErrorHideTimer=setTimeout(function(){autoSaveErrorHideTimer=0;if(autoSaveStatus&&autoSaveStatus.dataset.state==="error"){bar.hidden=true;}},9000);}
function setAutoSaveStatus(message,state,showFallback){if(autoSaveStatus){autoSaveStatus.textContent=message;autoSaveStatus.dataset.state=state||"ready";}if(downloadChanges){downloadChanges.hidden=!showFallback;}if(state==="error"){showAutoSaveErrorBriefly();}else if(autoSaveErrorHideTimer){clearTimeout(autoSaveErrorHideTimer);autoSaveErrorHideTimer=0;}}
function updateChangeBar(){var count=document.querySelectorAll('.core-card[data-has-change="true"]').length,confirmedCount=document.querySelectorAll('.core-card[data-confirmed="true"]').length,hasReview=count>0||confirmedCount>0,bar=document.getElementById("changeBar"),label=document.getElementById("changeCount"),errorVisible=autoSaveStatus&&autoSaveStatus.dataset.state==="error",canSave=!!corealignAppHub||!!projectFolderHandle||correctionBridgeReady;if(bar){bar.hidden=!hasReview||(errorVisible&&autoSaveErrorHideTimer===0);}if(label){label.textContent=count>0?count+" angle change"+(count===1?"":"s"):"Review checked";}if(openQuPath){openQuPath.hidden=!correctionOpenQuPathUrl;}if(hasReview&&autoSaveStatus&&autoSaveStatus.dataset.state==="ready"){setAutoSaveStatus(count>0?(canSave?"Saved in the project folder. Run CoreAlign when ready.":"Choose the project folder once to save."):"Checked. No rerun is needed unless an angle changed.",count>0&&!canSave?"error":"ready",count>0&&!canSave);}}
...
async function autoSaveCorrections(showModal){setAutoSaveStatus("Saving...","saving",false);var text=JSON.stringify(reviewCorrectionsPayload(),null,2)+String.fromCharCode(10);try{if(await writeProjectText("corealign-review-corrections.json",text,showModal===true)){setAutoSaveStatus("Saved in the project folder. Run CoreAlign when ready.","ready",false);if(showModal){showSavedModal("Rotation saved","The correction is in the project folder. Open QuPath and run CoreAlign when you are ready to update the image.");}return true;}}catch(error){if(error&&error.name==="AbortError"){setAutoSaveStatus("Choose the project folder once to save.","error",true);return false;}}if(correctionAutoSaveUrl){try{var response=await fetch(correctionAutoSaveUrl,{method:"POST",headers:{"Content-Type":"text/plain;charset=UTF-8"},body:text,cache:"no-store"});if(!response.ok){throw new Error("Save failed");}correctionBridgeReady=true;setAutoSaveStatus("Saved in the project folder. Run CoreAlign when ready.","ready",false);if(showModal){showSavedModal("Rotation saved","The correction is saved. Run CoreAlign when you are ready to update the image.");}return true;}catch(error){}}setAutoSaveStatus("Choose the project folder once to save.","error",true);if(showModal){showSavedModal("Folder needed","Choose the folder that contains REPORT.html. Use Chrome or Edge for a local file, or open the report in AppHub.");}return false;}
...
async function saveCorrectionFile(){if(await autoSaveCorrections(true)){return;}if(window.showDirectoryPicker||corealignAppHub){return;}var text=JSON.stringify(reviewCorrectionsPayload(),null,2)+String.fromCharCode(10),blob=new Blob([text],{type:"application/json"}),url=URL.createObjectURL(blob),link=document.createElement("a");link.href=url;link.download="corealign-review-corrections.json";link.click();setTimeout(function(){URL.revokeObjectURL(url);},0);setAutoSaveStatus("Downloaded. Move the file beside REPORT.html.","error",true);showSavedModal("Downloaded","Move corealign-review-corrections.json beside REPORT.html before running CoreAlign.");}
...
async function setOutputMode(mode){if(!resultsSection||mode===resultsSection.dataset.outputMode){return;}outputModeButtons.forEach(function(button){button.disabled=true;});if(outputModeStatus){outputModeStatus.textContent="Saving output mode...";}var saved=false;try{saved=await saveOutputModeToProject(mode,true);}catch(error){if(error&&error.name==="AbortError"){saved=false;}}if(!saved&&outputModeUrl){try{var response=await fetch(outputModeUrl,{method:"POST",headers:{"Content-Type":"text/plain;charset=UTF-8"},body:JSON.stringify({mode:mode}),cache:"no-store"});saved=response.ok;}catch(error){saved=false;}}if(saved){resultsSection.dataset.outputMode=mode;outputModeButtons.forEach(function(button){button.setAttribute("aria-pressed",button.dataset.outputModeChoice===mode?"true":"false");});if(outputModeStatus){outputModeStatus.textContent=mode==="research"?"Research saved. Run CoreAlign again to create OME-TIFF files.":"Presentation saved. Run CoreAlign again to update the package.";}showSavedModal(mode==="research"?"Research selected":"Presentation selected",mode==="research"?"The config is saved. Open QuPath and run CoreAlign when you are ready to create multichannel OME-TIFF files.":"The config is saved. Open QuPath and run CoreAlign when you are ready to update the presentation package.");}else{if(outputModeStatus){outputModeStatus.textContent="Choose the project folder once, then try again.";}showSavedModal("Folder needed","Choose the folder that contains REPORT.html and corealign.config.json. QuPath does not need to stay open.");}outputModeButtons.forEach(function(button){button.disabled=false;});}
...
if(downloadChanges){downloadChanges.addEventListener("click",saveCorrectionFile);}if(openQuPath){openQuPath.addEventListener("click",focusQuPath);}restoreProjectFolder().then(function(handle){projectFolderHandle=handle;updateChangeBar();if(handle&&document.querySelector('.core-card[data-has-change="true"]')){queueAutoSaveCorrections(false);}});if(corealignAppHub&&document.querySelector('.core-card[data-has-change="true"]')){queueAutoSaveCorrections(false);}
